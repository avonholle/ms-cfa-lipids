...
Proper model fit will guide suitability of these indicators for dyslipidemia construct in analyses. If measurement invariance across racial/ethnic or gender groups exists then there would also be no evidence of differences in the construct itself across these groups and allows for comparisons in means of the construct. Also, similarities across groups also implies the measured lipid indicators have similar pair-wise correlations. Conversely, measurement variance is an indicator that this latent factor is not the same across groups and should be treated as such with stratification.
...
CFA intercepts are parameters of interest for the best fitting model because they vary across groups in the metric specification of the model. In table 2, the intercepts for the confirmatory factor representing dyslipidemia are described by groups within each separate model. Within the model that separates by age, the three older age groups appear to have lower intercepts for HDL, with a standardized intercept resting at -0.163 for the 14 to 16 and 17 to 19 year age groups. For the middle age group, ages 14 to 16, LDL and TG intercepts are shifted further to the negative direction. In contrast, the 17 to 19 year old group have LDL and TG intercepts that are shifted closer to the origin when compared to the youngest age group.

For gender groups, the males appear to have a shift in the negative direction for HDL and towards the origin for TG when compared to females. Finally, the racial/ethnic groups also have differences in their CFA intercepts. Using the Mexican American (MA) group as a referent, the non-Hispanic Blacks (NHB) have a positive intercept for HDL unlike the majority of the other groups and the largest negative shift in the intercept for TG compared to the other groups. The non-Hispanic White group appears to have similar intercept values compared to the MA group. Finally, the other Hispanic (OH) group appears to have the largest  negative shift in LDL-C. One can think of the intercept for each subgroup to be the mean lipid outcome for the average dyslipidemia factor -- in this case when the dyslipidemia factor is zero since it is mean-centered in these analyses. These differences in intercepts for the groups have consequences for intergroup comparisons as will be discussed.

Finally, factor loadings are not allowed to vary across groups in the metric model. Standardized factor loadings without cross-loadings -- as in the specified analyses -- indicate the correlation between the dyslipidemia factor and the indicator while omitting measurement error. The TG indicator has the strongest correlation with the factor, at a correlation exceeding 0.71. HDL has the second strongest association -- around -0.52. Finally, LDL is the least correlated with the factor with a correlation at 0.39 for this sample. Any standardized factor loading below 0.3 indicates need to remove that indicator, and none of the lipid indicators fit that description. These values culminate in an interpretation of a one unit increase in the factor equating with an increase in unfavorable levels of lipids: declining HDL and increasing TG and LDL.

# Discussion

The purpose of this paper was to examine the use of CFA as a measure of dyslipidemia. First, we evaluated measures of model fit of a CFA with three common lipid measures serving as observed indicators in the model: HDL-C, LDL-C and TG. Nested within this aim is the concept that this CFA dyslipidemia construct may not be similar across sociodemographic groups such as race/ethnicity and age -- common ground for comparisons. Given known differences in the factor across these demographic categories, we tested this latter hypothesis through the measurement invariance (MI) concept. MI did not hold for different racial/ethnic nor to age groups indicating that the factor means can not be meaningfully compared across these groups, which has implications for future use.
...
Testing of measurement invariance may have intriguing implications. First, there is no evidence for differences in factor loadings across any subgroups of for race/ethnicity, gender and age. This finding supports the idea of a universal system in which the three lipid indicators interact no matter the choice of sociodemographic subgroup. Confounding may play less of a part in this explanation than that for the adult population as exposure to smoking and lifestyle choices has had less time to exact a cumulative toll. Also, a similarity of factor loadings across subgroups may have relevance for lipid ratio measures such as HDL to TG, a commonly used metric. For example, a dyslipidemia factor with positive HDL-C factor loadings combined with negative LDL-C and TG factor loadings translates to a simultaneous increase of HDL and decrease in LDL-C and TG. In turn this factor interpretation could be an analog to HDL-C to TG ratio changes, implying  generalizability of these ratios across sociodemographic subgroups. No research has been done comparing these measures and would be of interest.

Although the factor loadings for the construct are similar, we did not find evidence that metric invariance holds for these subgroups. Metric non-invariance implies at least one group has a different response to the lipid latent construct [@van_de_schoot_checklist_2012]. For example, when the factor is equal to zero for all subgroups, the estimated mean indicator (HDL-C, LDL-C and TG), the intercept, is different depending on the subgroup you choose. These multivariate findings, especially of the African American group having lower TG and higher HDL supports prior findings of univariate mean differences across racial/ethnic groups [@sumner_ethnic_2009; @kit_prevalence_2015].

Importantly, metric non-invariance for this factor restricts the use of mean factor scores across subgroups. Some researchers tend to obtain factor scores and use them as observed entities, followed by subgroup comparisons. These analyses demonstrate the perils in this approach for subgroup comparison of the dyslipidemia construct that result in scientifically meaningless estimates. Measurement non-invariance for all three domains supports different dyslipidemia constructs for each of the selected subgroups for race/ethnicity, age and gender. As such, comparisons in factor score means have different meanings and are not substantively meaningful. Although this finding remains restricted to this sample, it is of a nationwide representative sample of children in the United States and provides a cautionary tale of interpretation when conducting analyses in other samples.

## Fit

Finally, having chosen the metric model for all of the three domains, we determined that model fit diagnostics were within normal bounds of adequate to good fit and as such motivates further use of this construct. Use of this construct would be an interesting adjunct to univariate outcomes with a different and similarly meaningful interpretation.

Strengths of these analyses include the representation of the 12 to 19 year old population in the United States. Limitations include the possibility of a nonlinear association between these lipid indicators, which a CFA does not account for and may represent a mis-specified model. [need to add more to this section]
